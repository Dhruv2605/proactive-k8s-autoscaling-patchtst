"""
Cross-Domain Transfer Learning Evaluation (Fuzzy Feature Matching)
"""
import pandas as pd
import numpy as np
...
def fuzzy_get_columns(df, targets):
    cols = df.columns
    found = []
    for t in targets:
        match = [c for c in cols if t.lower() in c.lower()]
        if match:
            found.append(match[0])
            print(f"Mapped '{t}' to column '{match[0]}'")
        else:
            raise KeyError(f"Could not find fuzzy match for '{t}' in {cols.tolist()}")
    return found

# 2. Load Google Dataset
print("Loading Google Boutique training data...")
df_google = pd.read_csv("final_giant_thesis_dataset.csv")
google_cols = fuzzy_get_columns(df_google, ['qps', 'p50', 'p99', 'cpu'])
data_google = df_google[google_cols].dropna().values

# 3. Load Sock Shop Dataset
print("Loading Sock Shop evaluation data...")
df_sock = pd.read_csv("final_sockshop_dataset_final.csv")
sock_cols = fuzzy_get_columns(df_sock, ['Requests/s', '50%', '99%', 'cpu'])
data_sock = df_sock[sock_cols].dropna().values
