...
We generated high-fidelity datasets by subjecting the GKE cluster to continuous load for 127 hours across both benchmarks. 

\begin{itemize}
    \item \textbf{Online Boutique Dataset}: 28,108 observations were temporally aligned with Locust traffic logs using a 15-second tolerance window. This dataset captures 117 hours of varied traffic patterns.
    \item \textbf{Sock Shop Dataset}: 7,147 observations were collected over 10 hours, focusing on more complex service interactions and runtime-specific resource profiles.
\end{itemize}

The raw metrics were cleaned by removing idle periods and aligning asynchronous logs. The resulting features include Queries Per Second (QPS), P95 Latency (ms), Container Memory (bytes), and the target CPU cores. To prepare the data for the PatchTST model, we used a sliding window approach with a window size of 30 and a stride of 1, creating a training set of overlapping temporal snapshots. To prevent temporal data leakage, datasets were split chronologically rather than randomly: the 117-hour Boutique dataset and the 10-hour Sock Shop dataset were both split into the first 80\% for training, the next 10\% for validation, and the final 10\% for testing.
