"""
Cross-Domain Transfer Learning Evaluation (FIXED HEADERS)
...
# Handle literal single quotes in headers if present
df_google.columns = [c.strip().replace("'", "").replace('"', '') for c in df_google.columns]
...
df_sock.columns = [c.strip().replace("'", "").replace('"', '') for c in df_sock.columns]
# Mapping: qps -> Requests/s, p50 -> 50%, p99 -> 99%, cpu -> cpu
# Based on Step 2159: '50%', '99%', 'Requests/s', 'cpu'
